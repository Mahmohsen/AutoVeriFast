...
module VerifyProgram1(VerifyProgramArgs: VERIFY_PROGRAM_ARGS) = struct
 ...
 let print_and_return_termnode g = (*kfprintf (fun _ -> flush stdout) stdout "String %s" (ctxt#pprint g); fprintf stdout "\n";*) g
 let print_termnode g = ()(*kfprintf (fun _ -> flush stdout) stdout "String %s" (ctxt#pprint g); fprintf stdout "\n"*)
  

 let pprint_leaked_context_term t = 
    if options.option_simplify_terms then
      match ctxt#simplify t with None -> print_termnode t | Some(t) -> print_termnode t
    else
      print_termnode t

 let rec print_leaked_chunk_argument ts = match ts with 
   x :: [] -> pprint_leaked_context_term x;       
  |x :: y -> pprint_leaked_context_term x; print_leaked_chunk_argument y
   
 let return_leaked_context_term t = 
    if options.option_simplify_terms then
      match ctxt#simplify t with None -> ctxt#pprint t | Some(t) -> ctxt#pprint t
    else
      ctxt#pprint t
 ...
 let print_leaked_chunk h msg = 
       match h with
        (Chunk ((g, literal), targs, coef, ts, size)) :: [] -> kfprintf (fun _ -> flush stdout) stdout "This is the leaked chunk: "; print_termnode g; print_leaked_chunk_argument ts
       |(Chunk ((g, literal), targs, coef, ts, size)) :: y -> kfprintf (fun _ -> flush stdout) stdout "This is the leaked chunk: "; print_termnode g; print_leaked_chunk_argument ts
       
  let rec print_env env = match env with 
     (s, t) :: [] -> kfprintf (fun _ -> flush stdout) stdout "%s" s; kfprintf (fun _ -> flush stdout) stdout "%s" (ctxt#pprint t)
    |(s, t) :: x ->  kfprintf (fun _ -> flush stdout) stdout "%s" s; kfprintf (fun _ -> flush stdout) stdout "%s \n" (ctxt#pprint t); print_env x

 
 let rec create_leaked_heap h env (leaked_h : leaked_heap)= 
       match h with
        (Chunk ((g, literal), targs, coef, ts, size)) :: [] ->  
             (match leaked_h with 
              Leak_chunk ((s, a::b)) :: x  ->  Leak_chunk ((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: Leak_chunk ((s, a::b)) :: x
              |_ -> Leak_chunk ((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: [])       
       |(Chunk ((g, literal), targs, coef, ts, size)) :: y ->                  
             match leaked_h with 
               Leak_chunk ((s, a::b)) :: x  ->  Leak_chunk ((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: Leak_chunk ((s, a::b)) :: x
              |_ -> Leak_chunk ((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: []    
 ...
 let rec create_partial_heap h env= 
       match h with
        [] -> []
    |   (Chunk ((g, literal), targs, coef, ts, size)) :: y -> 
            Leak_chunk ((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: (create_partial_heap y env)
              
    
      
 let rec return_leaked_heap h env=
    match h with
        [] -> []
    |   Chunk((g, literal), targs, coef, ts, size) :: nextchunk ->
        Leak_chunk((ctxt#pprint g), (return_leaked_chunk_arguments ts)) :: (return_leaked_heap nextchunk env)
       
 let print_string name = kfprintf (fun _ -> flush stdout) stdout "%s " name
 
 let rec print_arg (arguments: string list) oc firstarg = 
    match arguments with
        x :: [] -> if(firstarg) then (output_string_file oc "(";  output_string_file oc x; output_string_file oc ")") else (output_string_file oc x; output_string_file oc ")")
        |x :: y -> if(firstarg) then (output_string_file oc "("; output_string_file oc x; output_string_file oc ","; print_arg y oc false) else (output_string_file oc x; output_string_file oc ","; print_arg y oc false)
     
 let check_predicate name = Str.split_delim (Str.regexp "[0..9]+") name
 let return_predicate_name names = 
    match names with
        x :: [] -> x
       |x :: y  -> x
...
let rec rev_list par revpar =
 ...
 let rec print_leaked_heap (leaked_h : leaked_heap) oc env =
    match leaked_h with 
        [] -> ()
   |    Leak_chunk (name, arguments) :: x ->  (output_string_file oc (return_predicate_name (check_predicate name))); (*(print_list (check_localname arguments env []));*) (print_arg arguments(*(rev_list (check_localname arguments env []) [])*) oc true) 
 ...
 let rec check_result_value leaked_h resultvalue = 
    match leaked_h with
        [] -> []
    |   Leak_chunk (name, arguments) :: x  -> Leak_chunk (name, (check_result_value_arg arguments resultvalue)) :: check_result_value x resultvalue  
...
let print_line l = kfprintf (fun _ -> flush stdout) stdout "%i " l; l 
 
let print_string_line l =  kfprintf (fun _ -> flush stdout) stdout "%s" l

let missing_heap msg = 
    match msg with 
        message -> String.sub message  (print_line ((String.index message ':') + 1)) ((print_line (String.length message)) - (print_line ((String.index message ':') + 1)))
...
let externalprint_pre file_lines file msg line_no =
let oc = open_out file in
    let rec modify i file_lines =
        match file_lines with
              [] -> close_out oc
            | line :: nextline -> 
                if (line = "\n") then             
                    ...
                    (output_string_file oc (new_pre_cond line); 
                    add_new_pre msg oc; 
                    output_string_file oc "\n"; 
                    modify (succ i) nextline) 
                else      
...
let externalprint_post_condition file_lines file msg line_no =
    let oc = open_out file in
        let rec modify i file_lines =
            match file_lines with
                [] -> close_out oc
...
let modifyline line x = 
    let index = try(Str.search_backward (Str.regexp (Printf.sprintf "%s%s" "?" x)) line (String.length line)) with Not_found -> -1 in
       if(index > -1) then
             (printnow "%s%i\n" "index " index; Printf.sprintf "%s%s%s%i%s" (String.sub line 0 index) "?" x 1 (String.sub line (index + 1 + (String.length x)) ((String.length line) - (index + (String.length x) + 1))))
        else
            line
    ...
    let rec modify i file_lines =
        ...
        |   line :: nextline -> 
                if (line = "\n") then 
                    ...
                    (output_string_file oc line; output_string_file oc msg; modify (succ i) nextline) 
...
(*End*)
 ...
 (*let search_context_stack_pre cs=  
  ...
  let search_context_stack_pre1 cs =
    let rec iter cs =
        match cs with
            [] -> 0
       |    Assuming t :: rest -> iter rest
...
let line_no l = 
    match l with
       ((s, r, col), (s1, r1, col1)) -> (kfprintf (fun _ -> flush stdout) stdout "%i " r)
...
let is_no_matching msg =
    if(String.length msg > 25) then
        let msg_first_part = (String.sub msg 0 25) in
            match msg_first_part with
                "No matching heap chunks: " -> (String.sub msg 25 ((String.length msg) - 25))
            |   _ -> "Other error type"    
    else
        "Other error type"
...
(*Any function ends with _test like the next function means that this funciton was just written for test purposes*)
let rec print_test env =
    match env with
        [] -> kfprintf (fun _ -> flush stdout) stdout "%s " "\n"
    |   (s, t) :: env -> 
            kfprintf (fun _ -> flush stdout) stdout "%s %s " s " - "; print_test env
            
let print_context_stack_test cs =
    match cs with
        Executing (h, env, l, msg) :: cs -> print_test env
    |   _   -> ()
...
(*The following functin return the line no. of first struct in the code where predicates would be written above*)
    let loc_of_firststruct =
        let rec iter ds line_no =
            match ds with
                [] -> line_no
            |   (sn, (((s,l,l1),(s1,l2,l3)), fds_opt))::dss ->
                if(line_no = 0) then
                    iter dss l
                else if (l < line_no) then
                    iter dss l 
                else 
                    iter dss line_no
        in iter structdeclmap 0 
  
  let checknormalfieldtype type_ =
        match type_ with
            Bool -> "Bool"
          | Void -> "Void"
          ...
          | ArrayType of type_ -> "Other"
          | StaticArrayType of type_ * int -> "Other" (* for array declarations in C *)
          | BoxIdType -> "Other" (* box type, for shared boxes *)
          | HandleIdType -> "Other" (* handle type, for shared boxes *)
          | AnyType -> "Other" (* supertype of all inductive datatypes; useful in combination with predicate families *)
          | TypeParam of string -> "Other" (* a reference to a type parameter declared in the enclosing datatype/function/predicate *)
          | InferredType of < > * type_ option ref -> "Other" (* inferred type, is unified during type checking. '< >' is the type of objects with no methods. This hack is used to prevent types from incorrectly comparing equal, as in InferredType (ref None) = InferredType (ref None). Yes, ref None = ref None. But object end <> object end. *)
          | ClassOrInterfaceName of string -> "Other" (* not a real type; used only during type checking *)
          | PackageName of string -> "Other" (* not a real type; used only during type checking *)
          | RefType of type_ -> "Other" (* not a real type; used only for locals whose address is taken *)
          | PluginInternalType of DynType.dyn -> "Other"
  *)
  let rec checkfieldtype fieldtype =
        match fieldtype with
            StructTypeExpr(l,s) -> s
          | PtrTypeExpr(l,t) -> checkfieldtype t
          | ArrayTypeExpr(l,t) -> checkfieldtype t
          | StaticArrayTypeExpr(l, t, i) -> checkfieldtype t
          | ManifestTypeExpr(l, t) -> checknormalfieldtype t  (* A type expression that is obviously a given type. *)
          | IdentTypeExpr(loc,Some (s1), s) -> s
  ...
  *)
          | PureFuncTypeExpr(l, (t::t1)) -> checkfieldtype t  
          | _ -> ""
    let rec print_pat pat =
        match pat with 
            LitPat(exp) ->  ""  (* literal pattern *)
        |   VarPat(l, s) ->  s  (* var pattern, aangeduid met ? in code *)
        |   DummyPat ->  ""
        |   CtorPat(l, s1, (pat2 :: pat3)) ->  ""
        |   WCtorPat(_,_,_,_,_,_,_) ->  ""
   ...
   let rec return_env contaxt =
    ...
    let rec generate_body body = 
        ...
        |   Sep(_,PredAsn(_,predref,_,[], (LitPat(Var(_,fieldname)):: restt)),asn2) ->
                if(predref#name = "chars") then
                begin
                    match restt with 
                        LitPat(Var(_,fieldname1)) :: z ->  Inner_pred(predref#name, (fieldname :: fieldname1 :: "_" :: print_otherpat z )) :: generate_body asn2
                    |   LitPat(CastExpr (l, trunc, te, e)) :: z -> 
                        begin
                            match e with
                            Operation(a,b,c) -> 
                            begin
                                match c with 
                                |   Var(_,fieldname2) :: Var(_, fieldname3) :: k -> Inner_pred(predref#name, (fieldname :: (Printf.sprintf "%s%s%s" fieldname2 "*" fieldname3) :: "_" :: print_otherpat restt)) :: generate_body asn2
                            end
                        end                    
               (*     |   [] -> printnow "%s\n" "something wrong"; Inner_pred(predref#name, (fieldname :: print_otherpat restt)) :: generate_body asn2*)
                end 
                else               
  ...
  let rec test_print_predicates predmap =
    match predmap with
        [] -> ()
    |   (name, param, body) ::  rest -> test_print_predicates rest
  ...
  let create_new_struct_map newstructmap s =
    let rec iter ds = 
        match ds with
            [] -> newstructmap
        |   (sn, (l, fds_opt))::dss ->
...
let searchpointer s str1=
         let rec iter i =
            if(i = 100) then false
            else
             let str = (Printf.sprintf "%s%s\n" "*" str1) in (printnow "%s%s\n" "THis is the *fieldname" str;
             if((try(Str.search_forward(Str.regexp str) (return_line s i) 0) with Not_found -> -1) > -1) then true
             else (iter (i+1)))
         in iter 0
  ...
  let ownership_list = 
    ...
    let rec check_struct_structure newstructmap =
        match newstructmap with
            [] -> newstructmap
        |   Structref(sn, allfields, None) :: rest -> 
            let rec iter fields=
                match fields with
                    [] -> Structref(sn, allfields, None) :: (check_struct_structure rest)
                |   Structfields(ident, fieldname) :: fieldsrest -> 
                    if(ident = sn) then
                        Structref(sn, allfields, Some (Structure(fieldname, "Linkedlist"))) :: (check_struct_structure rest)
                    else 
                        (iter fieldsrest)
            in iter allfields
  ...
  let rec print_autogen_fields structname fields autogenmap oc structure file =
    ...
    |   Autogen(x, y, z) :: autogenmap ->
            ...
            |   None ->              
                    ...
                    begin
                        if(check_struct_exist y (create_new_struct_map [] file) ) then
                        begin
                            output_string_file oc "&*& "; 
                            output_string_file oc y; 
                            output_string_file oc "("; 
                            ...
                            print_autogen_fields structname fields autogenmap oc structure file
                        end
                        else
                            ()
                    end
  ...
  let rec check_autogencounter autogencountermap oc structname autogenmap = 
    match autogencountermap with
        [] -> begin output_string_file oc ";"; output_string_file oc "\n" end
  ...
  let print_inner_structure structname structure autogenmap oc filename line_no =
    match structure with
        Some (Structure (x, "Linkedlist")) ->
            ...
            output_string_file oc ", ?count1) &*& count == count1 + 1; \n" (*&*& count > 0; \n*)
    |   None -> 
            (check_contain_llist autogenmap structname oc 1)

  let rec print_string_predicate fields oc s =
    match fields with
        [] -> ()
    |   Structfields(iden, fieldname) :: fields ->
            if(iden = "char" && searchpointer s fieldname) then
                begin
                output_string_file oc " &*& ";
                output_string_file oc "string1(";
                output_string_file oc fieldname;
                output_string_file oc ")";
                print_string_predicate fields oc s
                end
            else
                print_string_predicate fields oc s     
  
  let print_predicate_body structname fields autogenmap structure oc file line_no = 
    print_all_fields structname fields oc;
    print_string_predicate fields oc file; (*This function ony checks if there is a field of type pointer to char and create a predicate of type string for it*) 
    print_autogen_fields structname fields (*(ownership_list)*) autogenmap oc structure file; 
    print_inner_structure structname structure autogenmap oc file line_no
    

  
  let print_predicate_parameters structname fields autogenmap structure oc file line_no = 
    ...
    match structure with
        Some (Structure(_, "Linkedlist")) ->  
            ...
            print_predicate_body structname fields autogenmap structure oc file line_no
    |   None ->   
  ...
  let generate_predicate autogenmap oc newstructmap file line_no= 
    if(newstructmap <> []) then begin
        ...
        |    Structref(structname, fields, structure) :: nextnewstructmap -> 
                ...
                output_string_file oc " "; 
                output_string_file oc structname; 
                print_predicate_parameters structname fields autogenmap structure oc file line_no; 
                iter nextnewstructmap
        in iter newstructmap end
   else output_string_file oc ""      
      ...
      let rec modify i file_lines =
          match file_lines with
              ...
              | line :: nextline -> 
                  ...
                  begin
                      begin 
                      generate_predicate (*(autgendeclmap)*) (ownership_list) oc (check_struct_structure (create_new_struct_map [] file)) file line_no
                      end; 
                      output_string_file oc "\n\n";
                      output_string_file oc line; 
                      modify (succ i) nextline 
  ...
  let rec check_existance_heap predicate_body l_heap = 
    ...
    |   Leak_chunk(name, parameters) :: rest_heap ->
            match predicate_body with
                Name_ref(structname, fieldname, newreferencename) -> 
 ...
 let remove_encapsulated_chunk predbody l_chunk chunkname =
    let rec iter predbody =
        match predbody with
            [] -> l_chunk
        |   Name_ref(structname, fieldname, newreferencename) :: predicatebody -> 
                if(chunkname = (String.concat  "_" (structname :: fieldname :: []) )) 
                    then Leak_chunk("",("" :: [])) (*Adding dummy leaks*)
                    else iter predicatebody
        |   Inner_pred (innerpredicatename, paramters) :: predicatebody ->
                if(chunkname = innerpredicatename) 
                    then Leak_chunk("",("" :: [])) (*Adding dummy leaks*)
                    else iter predicatebody
    in iter predbody
 ...
 let rec remove_dummy_leaks l_heap =
    ...
    |   Leak_chunk(name, parameters) :: l_heap ->
            match name with 
                "" -> remove_dummy_leaks l_heap
            |   _  -> Leak_chunk(name, parameters) :: (remove_dummy_leaks l_heap)  
 ...
 let rec auto_close_predicate (l_heap: leaked_heap) (predmap: predicate_map) =
    ...
    |   (predicatename, predicateparam, predicatebody) :: predmap -> ((printnow "%s%s\n" "inside auto_close_predicate:  " predicatename);
            if(consume_leak_intopredicate l_heap predicatebody) then
                predicatename
            else auto_close_predicate l_heap predmap)
...
(*The followoing function just take the predicate name and look at the heap to exact the struct name that is represented by this predicate name. It does that by checking the malloc_block struct heap chunk*)
 ...
 let check_closed_pred leak_copy =
    match leak_copy with
        [] -> ""
    |   Leak_chunk(name, parameters) :: rest_heap ->
            match parameters with
                [] -> ""
            |   structname :: rest -> check_local_name structname (return_env !contextStack)

 let rec malloc_name leak leak_copy predicatename=
    match leak with
        [] -> check_closed_pred leak_copy
    |   Leak_chunk(name, parameters) :: rest_heap ->
            if((String.length name) > 12) then
                if((String.sub name 0 12) = "malloc_block") then
                    begin                    
                    if((String.sub name 13 ((String.length name) - 13)) = predicatename) then
                        match parameters with
                            structname :: [] -> structname                    
                    else
                        malloc_name rest_heap leak_copy predicatename
                    end
                else
                    malloc_name rest_heap leak_copy predicatename
            else
...
(* let rec check_local_name01 name env local_list  =
    match env with
        [] -> local_list
    |   (s, t) :: env -> (printnow "%s %s %s\n\n" name (ctxt#pprint t) s);
            if (name = (ctxt#pprint t)) then
...
*)
                ...
                (match l with
                   ((s,r,c),(s1,r1,c1)) ->
                    (if(row = r) then
                        ...
                        (check_local_name0 name cs row)))
            in iter_context row env)                           
    |   _ :: cs -> check_local_name0 name cs row
      

 let check_ghostlocal_name name cs row flag= 
    match name with
        "_" -> if(flag == 0) then
                    ""
               else
 ...
 let rec exist_local param env =
    match env with
        [] -> false
    |   (s,t) :: rest -> 
            if((ctxt#pprint t) = param) then
                true
            else
...
let rec search_context_stack_heap context =
    match context with
        [] -> []
...
let check_missingheap predname targs parameters env l =
       let rec iter parameters =
        match parameters with
            [] -> ()
        |   x :: parameters -> iter parameters
    in iter parameters        
        

let rec check_locality infered_count cs row = 
    match cs with
        [] -> false
    |   Executing (h, env, l, msg) :: cs -> begin
            match l with
                ((s1,r1,c1),(s2,r2,c2)) -> 
                         let rec iter env =
                            match env with
                                [] -> false(*(check_locality infered_count cs row)*)
                            |   (s, t) :: env ->
                                    if (infered_count = ctxt#pprint t) then
                                        if(row = r1) then true else (iter env)                         
                                    else
                                        (iter env)
                        in iter (remove_dups env)
            end                           
    |   _ :: cs -> (check_locality infered_count cs row)
   

let rec create_assumptions_list cs =
    match cs with
        [] -> []
    |   Assuming t :: cs -> Assuming t :: (create_assumptions_list cs)
    |   _   :: cs -> create_assumptions_list cs
    
let rec create_assum_exp assumptions =
    match assumptions with
        [] -> []
    |   Assuming t :: assumptions ->
            if(try (String.contains (ctxt#pprint t) '+') with _ -> false && try (String.contains (ctxt#pprint t) '=') with _ -> false) then
                let assumption = 
                    String.trim (ctxt#pprint t)
                in ((try (String.sub assumption 0 ((String.index assumption '=') - 1)) with _ -> (ctxt#pprint t)), (try (String.sub assumption ((String.index assumption '(') + 1) ((String.index assumption '+') - (String.index assumption '(') - 2)) with _ -> ""), (try (String.sub assumption ((String.index assumption '+') + 1) ((String.index assumption ')') - (String.index assumption '+') - 1)) with _ -> "")) :: (create_assum_exp assumptions) 
            else
                create_assum_exp assumptions

    
let rec solve_assump_equations assumptions_list infered_count (counter: int) cs row =
    match assumptions_list with
        [] -> (if(check_locality infered_count cs row) then 
                (Printf.sprintf "%s%s%i" infered_count "-" counter) 
               else if (infered_count = "0") then
                infered_count
               else                
                (Printf.sprintf "%s%s" "?" infered_count ))
...
let rec check_assumptions infered_count cs =
    match cs with
        [] -> infered_count
    |   Assuming t :: contextStack -> ctxt#pprint t
    |   _ :: contextStack -> check_assumptions infered_count contextStack
        
let check_infered_conut infered_count cs row  = (printnow "This is infered count %s\n" infered_count);
    if((try (Str.search_forward (Str.regexp "[a-z]+[0-9]*_*+") infered_count 0) with Not_found -> -1) >= 0) then 
        if(check_locality (Str.matched_string infered_count) cs row) then
            (check_local_name infered_count (return_env cs))
        else
        begin
            let new_infered_count = (Str.matched_string infered_count) in
            if((String.length infered_count) > (String.length new_infered_count)) then
                (solve_assump_equations (create_assum_exp (create_assumptions_list !contextStack)) new_infered_count ~-1 cs row)
            else
                solve_assump_equations (create_assum_exp (create_assumptions_list !contextStack)) new_infered_count 0 cs row
        end
    else
       ((printnow "%s\n" "I am hereeeeee"); 
       (*"0"*) infered_count)
...
let rec print_heap heap =
    match heap with
     [] -> ()
    |  Leak_chunk(predicatename, parameters) :: rest -> printnow "%s%s\n" "oook  " predicatename; print_heap rest
...
let rec auto_close_stmt heap_leak l env msg= printnow "%s\n" "Before ";print_heap heap_leak;printnow "%s\n" " after";
    if(autofix) then
        match l with
            ((s, r, col), (s1, r1, col1)) ->
...
let modify_postcondition s row col1 col2 infered_count wrong_count =
    let lines =
        lines_from_files s
    in
    let rec modify i row lines =
        match lines with
            [] -> []
        |   line :: lines -> 
                if (i = (row -1)) then                 
                    let col =
                        (*The flowing line ensures that the wrong_count is a seperate word, not part of other word*)
                        let wrong_count0 = Printf.sprintf "%s%s" " " wrong_count and wrong_count1 = Printf.sprintf "%s%s" "," wrong_count in ((printnow "%s %s\n" line wrong_count1);
                        try ((Str.search_forward (Str.regexp wrong_count1) line col2) + 1) with
                            Not_found -> 
                                try ((Str.search_forward (Str.regexp wrong_count0) line col2) + 1) with
                                    Not_found -> -1)
                    in
                    let modifiedline = ((printnow "Modifiedline %i\n" col);
                        if(col = -1) then 
                            line 
                        else
                            Printf.sprintf "%s%s%s" (String.sub line 0 (col)) (check_infered_conut infered_count !contextStack (row)) (String.sub line (col+(String.length wrong_count)) ((String.length line) - (col+(String.length wrong_count)))))
                    in 
                        modifiedline :: lines
                else
                    line :: (modify (i+1) row lines)
    in modify 0 row lines
                    
let print_modified_file lines s =
    let oc = open_out s in
        let rec update_file lines =
            match lines with
                [] -> close_out oc
            |   line :: lines -> (if (line = "\n") then (output_string_file oc "\n") else (output_string_file oc line; output_string_file oc "\n")); update_file lines
        in update_file lines
        
       
           
    
    
let update_postcondition_counter wrong_count infered_count l=
    if(autofix) then
        match l with
            ((s, r1, col1), (s1, r2, col2)) -> print_modified_file (modify_postcondition s r1 col1 col2 infered_count wrong_count) s
   

        
let rec parameters_list parameters row = 
    match parameters with
        [] -> []
    |   x :: parameters -> ((check_ghostlocal_name x !contextStack row 0) :: (parameters_list parameters row))
    
let rec parameters_list0 parameters row flag predicateparam env= 
    match parameters with
        [] -> (match predicateparam with
                [] -> []
              | (t, y) :: rest -> (Printf.sprintf "%s%s" "?" y) :: (parameters_list0 [] row flag rest env) )                
        |   x :: parameters -> 
            match predicateparam with
                [] -> ((printnow "%s" "hhheeerree\n");  
                    if((try(Str.search_forward (Str.regexp "^count") x 0) with Not_found -> -1) > -1) then                   
                        (check_infered_conut x !contextStack row) :: (parameters_list0 parameters row flag [] env)
                    else if(x = "_") then 
                        []   (*This should return unempty list if we are checking a chunk and not a predicate*)                     
                       (* "" :: parameters_list0 parameters row flag predicateparam*)
                    else
                        (check_local_name x env) :: parameters_list0 parameters row flag predicateparam env
                     (*   let parm_name = (check_ghostlocal_name x !contextStack row 1)
                         in
            ...
            |   (t, y) :: rest -> ((printnow "%s%s%s\n" x "& " y);
                    if((try(Str.search_forward (Str.regexp "^count") x 0) with Not_found -> -1) > -1) then
                        begin 
                        (check_infered_conut x !contextStack row) :: (parameters_list0 parameters row flag rest env)
                        end
                    else
                        let parm_name = (check_ghostlocal_name x !contextStack row 1)
                        in
                            (printnow "%s%s" "I am again here   " parm_name);
                            if(flag && ((try(Str.search_forward(Str.regexp "old_") parm_name 0) with Not_found -> -1) > -1)) then
                            begin
                                (String.sub parm_name 4 ((String.length parm_name) - 4)) :: (parameters_list0 parameters row flag rest env)
                            end
                            else         
                                parm_name :: (parameters_list0 parameters row flag rest env))
...
(*If this is the case, it just included an open command before the error line*)
                    
(*let rec search_heap predname heap r parameters0 =
    match heap with
        [] -> ""
    |   Leak_chunk(predicatename, parameters) :: heap ->
        begin
                                    ...
                                    let predicateparam =                                       
                                            let rec iter predmap =
                                                ...
                                                |   (name, param, body) :: rest -> 
                                                        if(predicatename = name) then param
                                                        else iter rest)
                                            in (iter (print_predicates))
                                    in
                                    let mssg = (Printf.sprintf "\n//@open %s%s%s%s \n" predname "(" (String.concat "," (parameters_list0 parameters r true predicateparam) ");") 
                                end
                                mssg
                                else
                                    search_heap predname heap r parameters0
...
*)
let predicateparam predicatename=                                       
    let rec iter predmap =
        (match predmap with
            [] -> []
        |   (name, param, body) :: rest -> 
...
let rec search_heap predname heap r parameters0 env =
    match heap with
        [] -> ""
    |   Leak_chunk(predicatename, parameters) :: heap ->
        begin
            ...
            begin
                match parameters with
                    [] -> search_heap predname heap r parameters0 env
                |   par1 :: restparameters -> 
                    begin 
                        match parameters0 with
                            [] -> search_heap predname heap r parameters0 env
                            ...
                            begin                                
                                if(par1 = par2) then
                                   (*  Printf.sprintf "\n//@open %s%s%s%s \n" predname "(" (String.concat "," (parameters_list0 parameters r true [])) ");" *)
                                    Printf.sprintf "\n//@open %s%s%s%s \n" predname "(" (String.concat "," (parameters_list0 parameters r true (predicateparam predname) env)) ");"
                                else
                                    search_heap predname heap r parameters0 env
                            end
                    end
...
let extractpredicatename predname predmap0 =
...
let rec open_predicate_precondition predname predmap currentinst s =
    match predmap with
        [] -> ""
    |   (predicatename, parameters, body) :: rest -> 
        ...
        begin
            let rec iter body msg predname = 
                match body with
                    [] ->  let msg1 = (print_recursivepred (check_struct_structure (create_new_struct_map [] s)) msg predname) in
                ...
                |   Inner_pred(innerpredicatename, paramters) :: rest ->
                        if(innerpredicatename = (Printf.sprintf "%s%s" "malloc_block_" predname)) then 
                            iter rest (Printf.sprintf "%s%s%s%s%s &*& " msg innerpredicatename "(" currentinst ")" ) predname                         
                        else if(innerpredicatename = "chars") then
                        begin
                            match paramters with
                            |   x :: y :: z :: k -> printnow "%s %s %s\n" x y z; iter rest (Printf.sprintf "%s%s%s%s%s%s%s%s%s &*& " msg innerpredicatename "(" x "," y "," z ")" ) predname 
                            |   x :: y -> printnow "%s\n" "nothing"; iter rest (Printf.sprintf "%s%s%s%s%s%s%s%s%s &*& " msg innerpredicatename "(" x "," x "," x ")" ) predname 
                        end                        
                        else 
                            iter rest msg predname
            in iter body "" predname        
        end
        else open_predicate_precondition predname rest currentinst s

let add_conditionalif predname parameters =
    match (!contextStack) with
            Executing (h, env,loc, msg) :: contextrest ->
                begin
                    if( msg = "Consuming assertion") then 
                    begin
                            match loc with
                                ((s,r,col),(s1,r1,col1)) ->  
                                    if((try(Str.search_forward(Str.regexp (Printf.sprintf "%s%s" predname "(result)")) (return_line s ((print_line ((search_context_stack_pre !contextStack)))+1)) 0) with Not_found -> -1) > -1) then                   
                                    begin                                      
                                        let rec iter env =
                                            match env with
                                                [] -> false
                                            |   (s1, t) :: env ->
                                                    begin
                                                    if(s1 = "result" && (ctxt#pprint t) = "0") then 
                                                        begin                                                                                            
                                                        match parameters with
                                                            x :: y ->   
...
let check_missingheap_exist_post line message1 message2 =
    match line with
        "" -> (-1,"")
    | line -> if((search_forward0 line message1 0 0) > 0) then (((search_forward0 line message1 0 0)),"predicate already exist") else if((search_forward0 line message2 0 0) > 0) then ((search_forward0 line message2 0 0),"predicate already exist with old keyword") else (-1,"")

let modify_post_condition1 line message1 message2 lineno s =
    match (check_missingheap_exist_post line message1 message2) with
    |   (-1, "") -> ()(*May be this should later add hte predicate to the end of the postcondition*)
    |   (x, "predicate already exist") -> ()
    |   (x, "predicate already exist with old keyword") -> 
            let mess1 = (String.sub line 0 (x)) and mess2 = (String.sub line (x+(String.length message2)) ((String.length line) - (x+(String.length message2)))) in (externalprint_post_condition (lines_from_files s) s (Printf.sprintf "%s%s%s" mess1 message1 mess2) (lineno+1))
...
let rec upper_boundery i assumptions =
        match assumptions with
        |   Assuming t :: rest -> 
                let firstpart = String.sub (ctxt#pprint t) 0 ((String.length i)+2) in
                    if(firstpart = (Printf.sprintf "%s %s" i "<")) then
                        (String.sub (ctxt#pprint t) ((String.length i)+3) ((String.length (ctxt#pprint t)) - ((String.length i)+3)))
                    else
                        (upper_boundery i rest) 
        |   [] -> ""
     
let find_missing_pointsto predname targs parameters h env = 
 if(autofix) then begin                      
        ...
        in
        match context with
             Executing (h1, env1, loc1, msg1) -> match loc1 with ((a,b,c),(a1,b1,c1)) ->
             match loc1 with
                ((s, r, col), (s1, r1, col1)) ->
                    match parameters with
                    x :: y ->
                    let messg = Printf.sprintf "%s%s%s%s" predname "(&" x ",?start)" in
                         externalprint_pre (lines_from_files s) s messg (print_line ((search_context_stack_pre !contextStack)))                  
  end     
let find_missing_arraychunk h env l array_predsym  a  i =
    if(autofix) then begin                      
            let context =
                let rec recheck_context cont =
                 match cont with 
        ...
        match context with
             ...
             match loc1 with
                ((s, r, col), (s1, r1, col1)) ->
                    let messg = Printf.sprintf "%s%s%s%s%s" (check_ghostlocal_name i (!contextStack) r 0) ">= 0 &*&" (check_ghostlocal_name i (!contextStack) r 0) "<= " (check_ghostlocal_name (upper_boundery i (create_assumptions_list !contextStack))(!contextStack) r 0) in
                         externalprint_pre (lines_from_files s) s messg (print_line ((search_context_stack_pre !contextStack)))                  
  end
        ...
        match context with
             Executing (h1, env1, loc1, msg1) -> match loc1 with ((a,b,c),(a1,b1,c1)) ->
                ...
                if(msg1 = "Evaluating loop condition") then
                    match loc1 with
                        ((s, r, col), (s1, r1, col1)) -> 
                                    let messg = open_predicate_precondition (extractpredicatename predname (print_predicates)) (print_predicates) x s in
                                        externalprint_pre (lines_from_files s) s messg (print_line ((search_context_stack_pre !contextStack)))  
                else
                begin
                        ...
                        match l with
                            ((s, r, col), (s1, r1, col1)) ->
                                ...
                                if((try(Str.search_forward(Str.regexp (Printf.sprintf "%s%s" predicatename "_")) (return_line s (print_line ((search_context_stack_pre !contextStack))) ) 0) with Not_found -> -1) > -1)                                 then
                                  ...
                                  let message = (search_heap predicatename heap r parameters env) in
                                    if(message = "") then
                                        (let predicateparam =                                       
                                            let rec iter predmap = 
                                                match predmap with
                                            ...
                                            in iter (print_predicates)
                                       in
                                       let message = Printf.sprintf "%s%s%s%s" predicatename "(" (String.concat "," (parameters_list0 parameters r true predicateparam env)) ")" 
                                       and message1 = Printf.sprintf "%s%s%s%s" predicatename "(" (String.concat ",?" (parameters_list0 parameters r true predicateparam env)) ")"
                                       and message3 = Printf.sprintf "%s%s%s%s" predicatename "(old_" (String.concat "," (parameters_list0 parameters r true predicateparam env)) ")"
                                       in 
                                            ...
                                            |   x -> match (check_missingheap_exist_pre (return_line s (x+1)) message1) with
                                                       -1 -> printnow "%s\n" "I am here123123hohokk"; externalprint_pre (lines_from_files s) s message (print_line (x+1)); printnow "%s\n" message 
                                                     | _ -> modify_post_condition1 (return_line s (x+2)) message1 message3 (x+1) s))
                                    else
...
let rec close_tail heap_leak l env =
            ...
            |   Leak_chunk(predicatename, parameters) :: rest ->
                    ...
                    |   x :: y ->
                            let localname_env =
                                ...
                                in iter x env
                            in
                            match localname_env with
                                z -> 
                                    if((try(Str.search_forward(Str.regexp (Printf.sprintf "%s%s" "_" z)) (return_line s (print_line (((search_context_stack_post !contextStack) - 1))) ) 0) with Not_found -> -1) > -1) then
                                        true
                                    else
                                        false
 ...
 let rec printenv env =
    match env with 
        [] -> ()
    |   (s,t) :: rest ->  printenv rest

(* let rec fix_leak heap_leak l env remaining_heap msg =
    if(autofix) then
        match l with
            ((s, r, col), (s1, r1, col1)) -> 
                    match heap_leak with
                        [] -> auto_close_stmt remaining_heap l env msg
                    |   Leak_chunk(predicatename, parameters) :: rest ->
                            if((close_tail heap_leak l (return_env !contextStack))) then
                                auto_close_stmt heap_leak l env msg
                            else
                                let rec iter predmap = 
                                    match predmap with
                                        [] -> ((printnow "%s\n" "Cheeeck me2"); (match parameters with 
                                                        x :: y -> (printnow "%s\n%s\n" x (return_line s (print_line ((search_context_stack_pre1 !contextStack)))));
                                                            let index = try(Str.search_forward (Str.regexp (Printf.sprintf "%s%s" "&" x)) (return_line s (print_line ((search_context_stack_pre1 !contextStack)))) 0) with Not_found -> -1 in 
                                                            if(index > -1) then                                                             (fix_leak rest l env ( Leak_chunk(predicatename, ((Printf.sprintf "%s%s" "&" x) :: y)) :: remaining_heap) msg) 
                                                            else ((printnow "%s\n" "Cheeeck me4");
                                                                if((try(Str.search_forward(Str.regexp predicatename) (return_line s (print_line ((search_context_stack_post !contextStack) - 1)) ) 0) with Not_found -> -1) > -1) then ((printnow "%s\n" "Cheeeck me5");                                                     
                                                (externalprint_post (lines_from_files s) s (update_predparam (updateheap_withlocalname (filter_lheap (check_leaked_returned_value env (Leak_chunk(predicatename, parameters) :: [])) (print_predicates)) env) [] env msg s r) (search_context_stack_post !contextStack) env; fix_leak rest l env remaining_heap msg)))))      
                                    |   (name, param, body) ::  predmaprest -> ((printnow "%s\n" "Cheeeck me3");
                                                    if(name = predicatename || predicatename = "string1") then 
                                                        match parameters with 
                                                        x :: y ->
                                                            let index = try(Str.search_forward (Str.regexp (Printf.sprintf "%s%s" "&" x)) (return_line s (print_line ((search_context_stack_pre1 !contextStack)))) 0) with Not_found -> -1 in 
                                                            if(index = -1) then                                                         
                                                                (fix_leak rest l env ( Leak_chunk(predicatename, parameters) :: remaining_heap) msg) 
                                                            else
                                                                (fix_leak rest l env ( Leak_chunk(predicatename, ((Printf.sprintf "%s%s" "&" x) :: y)) :: remaining_heap) msg)  
                                                    else
                                                        iter predmaprest)
                                in iter (print_predicates)     *)                
                                
 let rec fix_leak_old heap_leak l env remaining_heap msg = printnow "%s\n" "Before11 ";print_heap heap_leak;printnow "%s\n" " after22";
    if(autofix) then
        match l with
            ((s, r, col), (s1, r1, col1)) -> 
                    match heap_leak with
                    ...
                    |   Leak_chunk(predicatename, parameters) :: rest -> 
                            if((close_tail heap_leak l (return_env !contextStack))) then
                                ...
                                let rec iter predmap = 
                                    match predmap with
                                        [] -> if((try(Str.search_forward(Str.regexp predicatename) (return_line s (print_line ((search_context_stack_post !contextStack) - 1)) ) 0) with Not_found -> -1) > -1) then 
                                                (externalprint_post (lines_from_files s) s (update_predparam (updateheap_withlocalname (filter_lheap (check_leaked_returned_value env (Leak_chunk(predicatename, parameters) :: [])) (print_predicates)) env) [] env msg s r) (search_context_stack_post !contextStack) env; fix_leak_old rest l env remaining_heap msg) 
                                              else
                                                (fix_leak_old rest l env ( Leak_chunk(predicatename, parameters) :: remaining_heap) msg) 
                                    |   (name, param, body) ::  predmaprest ->
                                            if(name = predicatename || predicatename = "string1" || predicatename = "chars") then 
                                                (fix_leak_old rest l env ( Leak_chunk(predicatename, parameters) :: remaining_heap) msg) 
                                            else
                                                iter predmaprest
                                in iter (print_predicates)    

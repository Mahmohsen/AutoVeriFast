...
module Assertions(VerifyProgramArgs: VERIFY_PROGRAM_ARGS) = struct
  ...
  let lookup_points_to_chunk h0 env l f_symb t =
    match lookup_points_to_chunk_core h0 f_symb t with
      None -> (find_missing_pointsto (ctxt#pprint f_symb) "" ((ctxt#pprint t)::"_" :: []) h0 env); assert_false h0 env l ("No matching pointsto1 chunk: " ^ (ctxt#pprint f_symb) ^ "(" ^ (ctxt#pprint t) ^ ", _)") None
    | Some v -> v
    ...
    match slices with
      None ->
        begin match lookup_points_to_chunk_core h predsym (ctxt#mk_add a (ctxt#mk_mul i (sizeof l tp))) with
          None -> (find_missing_arraychunk h env l (ctxt#pprint array_predsym) (ctxt#pprint a) (ctxt#pprint i));
    ...
    let rec consume_chunk_core_core h =
      ...
      match iter [] h with
        ...
        begin fun cont ->
          ...
          match g with
            ...
            let try_rules rules ts = 
              ...
              (*let _ = print_endline ("#rules for " ^ (ctxt#pprint g) ^ "=" ^ rules) in*)
              let rec iter rules =
        ...
        let message =
          ...
          (find_missingheap predname targs (List.map string_of_pat0 pats) h env);

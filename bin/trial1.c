...
predicate tree1(struct tree *t, anytree b)
  requires switch(b){
    case NULL: return t==0;
    case tree1(a,bl,br): return t->value |-> ?v &*& t->left |-> ?l &*& t->right |-> ?r
    			&*& malloc_block_tree(t) &*& tree1(l,bl) &*& tree1(r,br) &*& t!=0 &*& a==v;};


predicate list(struct list *l, anylist anyl)
  requires switch(anyl){
    case emptylist: return l==0;
    case list(a,cons): return l->value |-> ?v &*& l->next |-> ?n &*& malloc_block_list(l) &*& list(n,cons) &*& a==v;};


inductive anytree = |NULL |tree1(int, anytree, anytree);

inductive bintree = |empty |tree(int,bintree,bintree);

inductive anylist = |emptylist |list(int,anylist);

fixpoint bool l_contains(anylist l, int v) {
  switch (l) {
    case emptylist: return false;
    case list(a,n): return a==v || l_contains(n, v);
  }
}

fixpoint bool l_order(anylist l, int u, int v) {
  switch (l) {
    case emptylist: return false;
    case list(a, n): return ((a==u && l_contains(n,v)) || l_order(n, u, v));
  }
}

fixpoint bool tr_contains(anytree b, int v) {
  switch (b) {
    case NULL: return false;
    case tree1(a,l,r): return a==v || tr_contains(l,v) || tr_contains(r,v);
  }
}

fixpoint bool tr_contains_l(anytree b, int v, int u) {
  switch (b) {
    case NULL: return false;
    case tree1(a,l,r): return ((a==u && tr_contains(l,v)) || tr_contains_l(l, v, u));
  }
}

fixpoint bool tr_contains_r(anytree b, int v, int u) {
  switch (b) {
    case NULL: return false;
    case tree1(a,l,r): return ((a==u && tr_contains(r,v)) || tr_contains_r(r, v, u));
  }
}

fixpoint bool tr_contains_nlr(anytree b, int v, int u) {
  switch (b) {
    case NULL: return false;
    case tree1(a,l,r): return ((a==u || a==v) ? false : (tr_contains_l(l, u, a) && tr_contains_r(r, v, a)));
  }
}

fixpoint bool tr_contains_nrl(anytree b, int v, int u) {
  switch (b) {
    case NULL: return false;
    case tree1(a,l,r): return ((a==u || a==v) ? false : (tr_contains_l(l, v, a) && tr_contains_r(r, u, a)));
  }
}
...
bool nb_contains(struct tree *t,int x)
  //@ requires tree1(t,?b);
  //@ ensures tree1(t,b) &*& result==tr_contains(b,x);
{
  if(t==0){
    //@ open tree1(t,b);
    //@ close tree1(t,NULL);
    ...
    if(v==x){
      ...
      bool temp1=nb_contains(l,x);
  //    //@close tree1(t,b);
      bool temp2=nb_contains(r,x);
      //@close tree1(t,b);
      return (temp1 || temp2);
  }
}

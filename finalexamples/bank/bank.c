...
authenticate_result->user_account |-> ?user_account &*& authenticate_result->is_teller |-> ?is_teller &*& authenticate_result->real_name |-> ?real_name &*& malloc_block_authenticate_result(authenticate_result)  &*& string1(real_name); 

predicate transaction (struct transaction *transaction; int count) = 
 transaction == 0 ? count == 0 : transaction->next |-> ?next &*& transaction->counterparty_bank_account_id |-> ?counterparty_bank_account_id &*& transaction->amount |-> ?amount &*& transaction->comment |-> ?comment &*& malloc_block_transaction(transaction)  &*& string1(counterparty_bank_account_id) &*& string1(comment) &*& transaction(next, ?count1) &*& count == count1 + 1; 

predicate bank_account (struct bank_account *bank_account; int count) = 
 bank_account == 0 ? count == 0 : bank_account->next |-> ?next &*& bank_account->id |-> ?id &*& bank_account->owner |-> ?owner &*& bank_account->balance |-> ?balance &*& bank_account->transaction_count |-> ?transaction_count &*& bank_account->transactions |-> ?transactions &*& malloc_block_bank_account(bank_account)  &*& string1(id) &*& transaction(transactions, transaction_count)
 &*& bank_account(next, ?count1) &*& count == count1 + 1; 

predicate user_account (struct user_account *user_account; int count) = 
 user_account == 0 ? count == 0 : user_account->next |-> ?next &*& user_account->user_name |-> ?user_name &*& user_account->password |-> ?password &*& user_account->is_teller |-> ?is_teller &*& user_account->real_name |-> ?real_name &*& malloc_block_user_account(user_account)  &*& string1(user_name) &*& string1(password) &*& string1(real_name) &*& user_account(next, ?count1) &*& count == count1 + 1; 

predicate bank (struct bank *bank; int user_accounts_count1, int bank_accounts_count1) = 
bank->user_account_count |-> ?user_account_count &*& bank->user_accounts |-> ?user_accounts &*& bank->bank_account_count |-> ?bank_account_count &*& bank->bank_accounts |-> ?bank_accounts &*& malloc_block_bank(bank) &*& user_account(user_accounts, user_accounts_count1)&*& bank_account(bank_accounts, bank_accounts_count1) &*& user_accounts_count1 == user_account_count &*& bank_accounts_count1 == bank_account_count;

@*/ 


struct bank {
    int user_account_count; /*@ counts user_accounts @*/
    /*@ owns @*/ struct user_account *user_accounts; 
    int bank_account_count; /*@ counts bank_accounts @*/
    /*@ owns @*/ struct bank_account *bank_accounts;
};
...
struct bank_account {
    ...
    int balance;
    int transaction_count; /*@ counts transactions @*/
    /*@ owns @*/ struct transaction *transactions;
};
...
//@ ensures true;
{
    ...
    {
    //	//@open bank(bank);
    	////@open user_account(user_account);
        if (isTeller != 0) {
